...
dataset = ImageDataset("dataset/selfie2anime", _transforms, False, 'test')
for row in range(5):
	dsize = len(dataset)
	data = dataset[dsize-row]
...
optimizerG = torch.optim.Adam(
	itertools.chain(GAB.parameters(), GBA.parameters()),
	lr=LearningRate, betas=BetaTuple)
optimizerDA = torch.optim.Adam(DA.parameters(), lr=LearningRate, betas=BetaTuple)
optimizerDB = torch.optim.Adam(DB.parameters(), lr=LearningRate, betas=BetaTuple)
...
# ReplayBuffer class (old data memorizing)
class ReplayBuffer:
...
valDataLoader = DataLoader(
	ImageDataset(f"dataset/{DatasetName}", _transforms=_transforms, unaligned=True, mode='test'),
	batch_size=4,
	shuffle=True,
)
...
def sampleImages(batchesDone):
	...
	with torch.no_grad():
		...
		realA = make_grid(realA, nrow=4, normalize=True)
		realB = make_grid(realB, nrow=4, normalize=True)
		fakeA = make_grid(fakeA, nrow=4, normalize=True)
		fakeB = make_grid(fakeB, nrow=4, normalize=True)

	imageGrid = torch.cat( (realA, fakeB, realB, fakeA), 1)
	save_image(imageGrid, f"images/{DatasetName}/{batchesDone}.png", normalize=True)
...
for epoch in range(InitEpoch, Epochs):
	for i, batch in enumerate(dataLoader):
		...
		recovA = GBA(fakeB)		
		recovB = GAB(fakeA)		
		lossCycA = criterionCycle(recovA, realA)
		lossCycB = criterionCycle(recovB, realB)
		lossCycle = (lossCycA+lossCycB)/2
		...
		lossD = (lossDA + lossDB)/2   # for print
