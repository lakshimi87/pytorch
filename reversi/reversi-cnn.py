...
Path = "./reversi-cnn.pth"

if os.path.exists(Path):
	model = torch.load(Path).to(device)
	print(f"Loading {Path} is succeeded")
else:
	model = ExValue().to(device)
...
class TreeNode:
	def __init__(self, parent, board, place, turn):
		...
		self.win = 0		  # 이긴 횟수
		self.visit = 0		# 방문 횟수
		_, self.hints = getHints(board, turn) # 놓을 수 있는 위치 표시
		random.shuffle(self.hints) # 놓는 위치를 다양하게 하기 위해
		self.children = []	# n-ary 트리를 만들기 위한 설정
		self.parent = parent
		if parent: parent.children.append(self)
		x = convertToInput(board)
		self.exValue = model(x).detach().to(cpudevice).numpy()[0, 0] * ((turn==1)*2-1)
	...
	def simulate(self):
		...
		while passCount < 2:
			...
			hintCount, hints = getHints(board, turn)
...
class MCTree:
	...
	def select(self, root=None):
		...
		for _ in range(SimulationCount):
			w, b = node.simulate()
			if (self.root.turn==1 and w>b) or (self.root.turn==2 and w<b): win += 1
		# back propagation
		while node != None:
			node.win += win
...
#	return prerun score when place p position with turn.
...
def Place(board, p, turn):
	...
	# 턴 바꾸기
	turn ^= 3
	hintCount, _ = getHints(board, turn)
...
# get next place by score board
def getNextPlaceByScoreBoard(board, turn):
	hints = [ i for i in range(64) if board[i] == 0 ]
	maxValue, maxPlace = -1000, -1
	for p in hints:
		score = getPrerunScore(board, p, turn)
		if maxValue < score: maxValue, maxPlace = score, p
	return maxPlace

# get next place by MCTS
def getNextPlaceByMCTS(board, turn):
	# create a new MCTree
...
for g in range(n):
	...
	while turn != 0:
		x = convertToInput(board)
		y = model(x).detach().to(cpudevice).numpy()[0]
		episode.append( (board, turn, y) )
		if turn == userTurn: p = getNextPlaceByScoreBoard(board, turn)
		else: p = getNextPlaceByMCTS(board, turn)
		if p == None:
			pygame.quit()
			quit()
	...
	for board, turn, v in episode:
		...
		x.append(np.array(xx).reshape(1, 8, 8))
		# MDP 계산
		v += (0.1)*(z - v)
		y.append(v)
